...
if (!pfxBase64 || !pfxPassword) {
  console.error("ERROR: STORE_PFX_BASE64 or STORE_PFX_PASSWORD environment variables are not set.");
  process.exit(1);
}
...
try {
  ...
  if (!signtoolPath) {
    console.error("ERROR: signtool.exe could not be found on the system.");
    process.exit(1);
  }
  ...
  for (const tsServer of timestampServers) {
    ...
    try {
      ...
      console.log(`SUCCESS: Binary signed successfully using ${tsServer}!`);
