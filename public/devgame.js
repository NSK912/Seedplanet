...
// --- ระบบวงรอบตัดไม้-ทุบหิน (Harvest Rings: Trees & Rocks Outlines) ---
(function initDevHarvestRingsControl() {
  if (typeof window.showHarvestRings === "undefined") {
    window.showHarvestRings = false;
  }
  if (typeof window.harvestRingsDistance === "undefined") {
    window.harvestRingsDistance = 8.0;
  }
  if (typeof window.harvestRingsAlwaysShow === "undefined") {
    window.harvestRingsAlwaysShow = false;
  }

  function mountGroup() {
    const mainControls = document.getElementById("mainControls");
    if (!mainControls) return false;
    if (document.getElementById("devHarvestRingsControlGroup")) return true;

    const group = document.createElement("div");
    group.className = "control-group";
    group.id = "devHarvestRingsControlGroup";
    group.style.border = "1.5px solid #4caf50";
    group.style.borderRadius = "8px";
    group.style.background = "rgba(10, 26, 14, 0.95)";
    group.style.boxShadow = "0 4px 12px rgba(0,0,0,0.5)";
    group.style.padding = "10px";

    group.innerHTML = `
      <div style="font-weight: bold; color: #66bb6a; font-size: 13px; margin-bottom: 6px; display: flex; justify-content: space-between; align-items: center;">
        <span>🪓 วงรอบตัดไม้-ทุบหิน (Harvest Rings)</span>
        <span id="devHarvestRingsBadge" style="font-size: 10px; padding: 2px 6px; border-radius: 4px; background: rgba(255,255,255,0.1); color: #888; font-weight: normal;">ปิด (Disabled)</span>
      </div>
      
      <div style="font-size: 11px; color: #c8e6c9; margin-bottom: 8px; line-height: 1.4;">
        แสดงเส้นวงรอบต้นไม้และหินที่ตัด/ทุบได้อัตโนมัติเมื่อถือ ขวาน (Axe) หรือ อีเตอร์ (Pickaxe) พร้อมไฮไลท์เป้าหมายในระยะทำการ
      </div>

      <button id="devHarvestRingsMainBtn" class="btn-random" style="width: 100%; margin: 0 0 8px 0; padding: 9px 8px; font-size: 12px; font-weight: bold; color: #fff; background-image: linear-gradient(135deg, #424242, #616161); border-radius: 6px; box-shadow: 0 2px 6px rgba(0,0,0,0.3); cursor: pointer; border: 1px solid rgba(255,255,255,0.2); transition: all 0.2s ease;">
        ❌ ปิดใช้งานวงรอบตัดไม้-ทุบหิน (OFF)
      </button>

      <div style="display: flex; gap: 6px; margin-bottom: 8px;">
        <button id="devHarvestGetAxeBtn" class="btn-random" style="flex: 1; margin: 0; padding: 6px 4px; font-size: 11px; font-weight: bold; background-image: linear-gradient(135deg, #37474f, #455a64); color: #fff; border-radius: 5px; cursor: pointer; border: 1px solid rgba(255,255,255,0.15); text-align: center;">
          🪓 รับขวาน (Get Axe)
        </button>
        <button id="devHarvestGetPickaxeBtn" class="btn-random" style="flex: 1; margin: 0; padding: 6px 4px; font-size: 11px; font-weight: bold; background-image: linear-gradient(135deg, #37474f, #455a64); color: #fff; border-radius: 5px; cursor: pointer; border: 1px solid rgba(255,255,255,0.15); text-align: center;">
          ⛏️ รับอีเตอร์ (Pickaxe)
        </button>
      </div>

      <div style="display: flex; justify-content: space-between; align-items: center; font-size: 11px; color: #a5d6a7; margin-bottom: 4px;">
        <span>📏 ระยะมองเห็นวงรอบ (View Distance):</span>
        <span id="devHarvestDistLabel" style="font-weight: bold; color: #fff;">8.0m</span>
      </div>
      <input type="range" id="devHarvestDistSlider" min="4" max="25" step="1" value="8" style="width: 100%; margin-bottom: 8px; cursor: pointer;" />

      <label style="display: flex; align-items: center; gap: 6px; font-size: 11px; color: #e8f5e9; cursor: pointer; user-select: none;">
        <input type="checkbox" id="devHarvestAlwaysShowCheck" style="cursor: pointer;" />
        <span>แสดงวงรอบตลอดเวลา (แม้ไม่ได้ถือเครื่องมือ)</span>
      </label>
    `;

    mainControls.appendChild(group);

    const mainBtn = group.querySelector("#devHarvestRingsMainBtn");
    const badge = group.querySelector("#devHarvestRingsBadge");
    const getAxeBtn = group.querySelector("#devHarvestGetAxeBtn");
    const getPickaxeBtn = group.querySelector("#devHarvestGetPickaxeBtn");
    const distSlider = group.querySelector("#devHarvestDistSlider");
    const distLabel = group.querySelector("#devHarvestDistLabel");
    const alwaysCheck = group.querySelector("#devHarvestAlwaysShowCheck");

    window.updateDevHarvestRingsUI = function() {
      const isEnabled = !!window.showHarvestRings;
      if (mainBtn && badge) {
        if (isEnabled) {
          mainBtn.style.backgroundImage = "linear-gradient(135deg, #2e7d32, #4caf50)";
          mainBtn.innerHTML = "✅ เปิดใช้งานวงรอบตัดไม้-ทุบหิน (ON)";
          badge.textContent = "เปิด (Active)";
          badge.style.background = "rgba(76, 175, 80, 0.25)";
          badge.style.color = "#81c784";
        } else {
          mainBtn.style.backgroundImage = "linear-gradient(135deg, #424242, #616161)";
          mainBtn.innerHTML = "❌ ปิดใช้งานวงรอบตัดไม้-ทุบหิน (OFF)";
          badge.textContent = "ปิด (Disabled)";
          badge.style.background = "rgba(255,255,255,0.1)";
          badge.style.color = "#888";
        }
      }

      const sideBtn = document.getElementById("harvestRingsToggle");
      if (sideBtn) {
        sideBtn.textContent = `⭕ แสดงวงรอบตัดไม้-ทุบหิน (Harvest Rings) ${isEnabled ? "เปิด" : "ปิด"}`;
        sideBtn.classList.toggle("active", isEnabled);
      }
    };

    if (mainBtn) {
      mainBtn.addEventListener("click", () => {
        window.showHarvestRings = !window.showHarvestRings;
        if (typeof showHarvestRings !== "undefined") showHarvestRings = window.showHarvestRings;
        if (typeof window.updateDevHarvestRingsUI === "function") window.updateDevHarvestRingsUI();
        if (typeof showNotice === "function") {
          showNotice(window.showHarvestRings ? "⭕ เปิดใช้งานวงรอบตัดไม้-ทุบหิน" : "❌ ปิดใช้งานวงรอบตัดไม้-ทุบหิน");
        }
      });
    }

    if (getAxeBtn) {
      getAxeBtn.addEventListener("click", () => {
        if (typeof addItemToInventory === "function") {
          addItemToInventory({ name: "AXE", icon: "🪓", label: "AXE" }, true, true);
        }
        if (typeof showNotice === "function") showNotice("🪓 ได้รับขวาน (AXE) เรียบร้อย!");
      });
    }

    if (getPickaxeBtn) {
      getPickaxeBtn.addEventListener("click", () => {
        if (typeof addItemToInventory === "function") {
          addItemToInventory({ name: "PICKAXE", icon: "⛏️", label: "PICKAXE" }, true, true);
        }
        if (typeof showNotice === "function") showNotice("⛏️ ได้รับอีเตอร์ (PICKAXE) เรียบร้อย!");
      });
    }

    if (distSlider && distLabel) {
      distSlider.value = window.harvestRingsDistance || 8;
      distLabel.textContent = parseFloat(distSlider.value).toFixed(1) + "m";
      distSlider.addEventListener("input", () => {
        window.harvestRingsDistance = parseFloat(distSlider.value);
        distLabel.textContent = window.harvestRingsDistance.toFixed(1) + "m";
      });
    }

    if (alwaysCheck) {
      alwaysCheck.checked = !!window.harvestRingsAlwaysShow;
      alwaysCheck.addEventListener("change", () => {
        window.harvestRingsAlwaysShow = alwaysCheck.checked;
        if (typeof showNotice === "function") {
          showNotice(window.harvestRingsAlwaysShow ? "👁️ แสดงวงรอบต้นไม้และหินตลอดเวลา" : "🪓 แสดงวงรอบเฉพาะเวลาถือขวาน/อีเตอร์");
        }
      });
    }

    if (typeof window.updateDevHarvestRingsUI === "function") {
      window.updateDevHarvestRingsUI();
    }
    return true;
  }
...
if (typeof window !== "undefined") {
  ...
  // ระบบเปิด-ปิดวงรอบเป็นเส้น ต้นไม้ หิน ที่สามารถ ตัด ทุบ ออกได้
  window.setHarvestRings = function(enabled) {
    const val = !!enabled;
    window.showHarvestRings = val;
    if (typeof showHarvestRings !== "undefined") {
      showHarvestRings = val;
    }
    if (typeof window.updateDevHarvestRingsUI === "function") {
      window.updateDevHarvestRingsUI();
    }
    const btn = document.getElementById("harvestRingsToggle");
    if (btn) {
      btn.textContent = `⭕ แสดงวงรอบตัดไม้-ทุบหิน (Harvest Rings) ${val ? "เปิด" : "ปิด"}`;
      btn.classList.toggle("active", val);
    }
    if (typeof showNotice === "function") {
      showNotice(val ? "⭕ เปิดแสดงวงรอบตัดไม้-ทุบหิน" : "❌ ปิดแสดงวงรอบตัดไม้-ทุบหิน");
    }
    return val;
  };
  window.toggleHarvestRings = function() {
    const current = (typeof window.showHarvestRings !== "undefined") ? window.showHarvestRings : false;
    return window.setHarvestRings(!current);
  };
}
