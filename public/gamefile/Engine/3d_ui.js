...
const World3DUI = {
  ...
  updateBoatUI(options) {
    ...
    const drawBoatUI = (ctx, w, h) => {
      ...
      // Draw progress bars
      if (isEngineBoat) {
        ctx.fillStyle = batteryPercent <= 50 ? 'rgba(225, 29, 72, 0.65)' : 'rgba(150, 150, 150, 0.65)';
        ctx.fillRect(p, p, innerW * (batteryPercent / 100), innerH);
      }
      ...
      // Draw text or icon
      if (isEngineBoat) {
        if (!window._cachedBatteryCanvasLarge) {
          window._cachedBatteryCanvasLarge = "pending";
          if (typeof create3DIconCanvas === 'function') {
            const c = create3DIconCanvas("GLOW_BATTERY", 128, 128);
            if (c) window._cachedBatteryCanvasLarge = c;
          }
        }
        if (window._cachedBatteryCanvasLarge && window._cachedBatteryCanvasLarge !== "pending") {
          const imgSize = w * 0.75;
          ctx.drawImage(window._cachedBatteryCanvasLarge, (w - imgSize) / 2, (h - imgSize) / 2, imgSize, imgSize);
        } else {
          ctx.fillStyle = '#fff';
  ...
  updateMechStandUI(options) {
    if (!this.backend) return;
    const { stand, parts = [], playerScale = 0.1 } = options;
    ...
    const drawMechStandUI = (ctx, w, h) => {
      ...
      ctx.fillStyle = 'rgba(223, 183, 108, 0.65)';
      ctx.font = `${Math.round(15 * scale)}px "JetBrains Mono", monospace`;
      ctx.textAlign = 'right';
      ctx.fillText('● 8 SLOTS DETECTED', w - 36 * scale, 42 * scale);
