...
(function(global) {
  ...
  function getWaterRadiusAt(x, y, z) {
    ...
    const maxGrid = Math.min(gridSize, 120);
  ...
  function buildWaterSphere(gridSize, wLevel, planetR) {
    ...
    const maxWaterSize = Math.min(gSize, 120);
    ...
    const waterRadius = rad + level * 0.15;
  ...
  function updateWaterMask() {
    ...
    const planetR = typeof RADIUS !== 'undefined' ? RADIUS : 8.0;
    const wLevel = typeof waterLevel !== 'undefined' ? waterLevel : 1.0;
    const seed = typeof globalSeed !== 'undefined' ? globalSeed : 12345;
    ...
    const latSeg = Math.min(gSize, 120);
    const longSeg = Math.min(gSize, 120);
