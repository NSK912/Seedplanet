...
(function(global) {
  ...
  function generateClouds(globalSeed, planetRadius, cloudsHeight) {
    ...
    const areaScale = (planetRadius / 8.0) ** 2;
    const numClouds = Math.floor(80 * areaScale);
    let currentIndex = 0;
    ...
    for (let i = 0; i < numClouds; i++) {
        ...
        let r = planetRadius + cloudsHeight + pseudoRandom(currentSeed++) * 0.8;
        ...
        for (let p = 0; p < numPuffs; p++) {
            let pRadius = 0.8 + pseudoRandom(currentSeed++) * 1.2;
            let ox = (pseudoRandom(currentSeed++) - 0.5) * 2.0;
            let oy = (pseudoRandom(currentSeed++) - 0.5) * 0.8;
            let oz = (pseudoRandom(currentSeed++) - 0.5) * 2.0;
        ...
        cloudChunks.push({
            position: [cx, cy, cz],
            radius: 8.0, // generous bounding radius to avoid premature frustum culling
    ...
    return {
        ...
        indices: new Uint16Array(indices), // Assuming < 65k vertices. With 60 clouds * 5 puffs * 60 vertices = 18,000 verts. Safe for Uint16.
        chunks: cloudChunks
    };
  }
