...
function initAmphibians(count, seed) {
  ...
  while (i < count) {
    ...
    const isLand = (height * HEIGHT_SCALE > waterLevel * 0.15);

    const r =
      RADIUS +
      Math.max(height * HEIGHT_SCALE, waterLevel * 0.15 - 0.05) +
      0.05 +
      Math.random() * 0.05;
    ...
    if (isLand) {
      if (rand < 0.4) type = 'human';
      else type = 'meganeura';
    } else {
      if (rand < 0.3) type = 'meganeura';
      else type = 'georgiacetus';
    }
    ...
    if (type === 'human') {
      ...
      for (let j = 0; j < groupSize && i < count; j++) {
        ...
        let offsetPhi = phi;
        if (j > 0) {
           offsetTheta += (Math.random() - 0.5) * 0.04;
           offsetPhi += (Math.random() - 0.5) * 0.04;
        }
        const oHeight = getHeightOnSphere(offsetTheta, offsetPhi, seed);
        if (oHeight * HEIGHT_SCALE <= waterLevel * 0.15 && j > 0) continue; // Must be on land
      ...
      amphibians.push({
        ...
        theta: theta,
        phi: phi,
        r: isMeganeura ? RADIUS + height * HEIGHT_SCALE + 0.3 : r,
        heading: Math.random() * Math.PI * 2,
        animPhase: Math.random() * Math.PI * 2,
        ragdollEnabled: false,
        ragdollInitialized: false,
...
function updateAmphibians(deltaTime, seed) {
  ...
  for (let c of amphibians) {
    ...
    if (c.ragdollEnabled) {
      if (c.ragdollPos) {
        ...
        if (dist > 5.0) {
          ...
          if (c.type === 'human') {
            ...
            for (let other of amphibians) {
               if (other.type === 'human' && other !== c && !other.ragdollEnabled) {
                   if (Math.random() < 0.5) {
                       newTheta = other.theta + (Math.random() - 0.5) * 0.04;
                       newPhi = other.phi + (Math.random() - 0.5) * 0.04;
                       newHeight = getHeightOnSphere(newTheta, newPhi, seed);
                       if (newHeight * HEIGHT_SCALE > waterLevel * 0.15) {
                           foundLeader = true;
                           break;
                       }
                   }
            ...
            if (!foundLeader) {
                for(let tries=0; tries<10; tries++) {
                   if (newHeight * HEIGHT_SCALE > waterLevel * 0.15) break;
          ...
          const newR = isMeganeura ? gRadius + 0.3 : Math.max(gRadius, wRadius - 0.02) + 0.05;
    ...
    const scale = c.type === 'meganeura' ? 0.25 : 0.5;
